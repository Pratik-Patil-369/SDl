     ...
     protected void paintComponent(Graphics g) {
        ...
        for (Shape shape : shapes) {
            ...
            if (shape.type == 0) {  // Circle
                g.fillOval(shape.x, shape.y, 30, 30);  // Draw circle with diameter 30
            } else if (shape.type == 1) {  // Rectangle
                g.fillRect(shape.x, shape.y, 30, 50);  // Draw rectangle with width 30 and height 50
            } else if (shape.type == 2) {  // Square
                g.fillRect(shape.x, shape.y, 30, 30);  // Draw square with width and height 30
            }
        }
    }
